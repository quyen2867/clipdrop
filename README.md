...
Bản Docker khi chạy chế độ public khởi động kèm **PO token provider bgutil** chỉ nghe trên loopback. Với link YouTube, worker ưu tiên client `visionos` (không cần PO token, phản hồi trong 1-2 giây, tải video và âm thanh lên đến 4K mà không bị lỗi 403), sau đó fallback sang `mweb` → `tv` (kèm PO token) rồi mới quay về client mặc định của yt-dlp. Hỗ trợ truyền cookie qua biến môi trường `CLIPDROP_COOKIES` và proxy qua `CLIPDROP_PROXY` trên Render để vượt qua mọi thử thách bot/yêu cầu đăng nhập của YouTube khi cần. Mỗi profile có trần riêng 20 giây và cả chuỗi có ngân sách 45 giây để không vượt timeout 90 giây của bước xem thông tin. Profile thắng được ghi nhớ và **dùng lại cho cả bước tải**. `CLIPDROP_YOUTUBE_CLIENTS` ép đúng danh sách client thay cho chuỗi mặc định (ví dụ `CLIPDROP_YOUTUBE_CLIENTS=visionos`). `/api/health` báo `pot_alive`, `cookies`, `proxy`, kèm `memory_mb`, `memory_limit_mb`, `oom_kills`.
