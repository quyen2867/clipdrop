...
Bản Docker khi chạy chế độ public khởi động kèm **PO token provider bgutil** chỉ nghe trên loopback. Với link YouTube, chế độ public thử lần lượt `android_vr` → `mweb` → `tv` (kèm PO token) rồi mới quay về client mặc định của yt-dlp: `android_vr` không cần token nên trả lời trong vài giây, còn tạo token tốn hơn nhiều (đo trên container giới hạn 0.1 CPU: `android_vr` 4 giây, `mweb` khoảng 35 giây; bản cũ trên Render còn tiêu hết ngân sách 40 giây mà không client nào trả lời). Bản local không bật `CLIPDROP_PUBLIC` vẫn thử `mweb` → `tv` → `android_vr` trước, vì client cần token liệt kê nhiều mức chất lượng hơn. Mỗi profile có trần riêng 20 giây và cả chuỗi có ngân sách 45 giây để không vượt timeout 90 giây của bước xem thông tin; một profile bị bỏ dở không còn chặn các profile sau. Profile thắng được ghi nhớ và **dùng lại cho cả bước tải**, vì URL media do client cần token trả về sẽ bị 403 nếu thiếu token. Provider **tự bật** khi image có sẵn nó (loopback `http://127.0.0.1:4416`), không cần khai báo env; đặt `CLIPDROP_POT_URL=0` để tắt, hoặc URL loopback khác để đổi địa chỉ. `CLIPDROP_YOUTUBE_CLIENTS` ép đúng danh sách client thay cho chuỗi mặc định (ví dụ `mweb,tv` để ưu tiên chất lượng cao). `/api/health` báo `pot_alive` (provider có thật sự trả lời `/ping`) kèm `memory_mb`, `memory_limit_mb`, `oom_kills` để phân biệt “YouTube từ chối” với “container hết RAM”. Provider không làm thay đổi bản local: chỉ chạy trong image Docker của repo.
...
deploy/             Khởi động container, PO token provider và FFmpeg chỉ đọc file local
Dockerfile          Python + Node + FFmpeg + PO token provider, chạy non-root
...
Đã kiểm tra trên Mac: 66 test tự động (chạy cả trong image Docker của bản deploy), bao gồm chặn kết nối mạng nội bộ sau DNS, cookie và phân tách file giữa hai phiên, quota gửi file đồng thời và hàng đợi, thứ tự chuỗi client YouTube theo chế độ chạy, trần thời gian riêng của từng profile, dùng lại profile khi tải, thăm dò provider và đọc RAM/OOM.
 Bản local trước đó đã tải thật Sintel trailer thành MP4 (H.264 + AAC) và MP3, ghép hai luồng bằng FFmpeg, kiểm tra giao diện tại chiều rộng 319 px và 1280 px. Khả năng tải từng nguồn vẫn tùy điều kiện truy cập hiện tại.
