...
- Với link YouTube, worker thử lần lượt client `mweb` → `tv` → `android_vr` kèm PO token rồi mới quay về client mặc định của yt-dlp; mỗi lần thử chỉ mất khoảng 2 giây. Chuỗi có ngân sách 40 giây: khi hết ngân sách, các client còn lại bị bỏ qua và log ghi `skipped` thay vì để worker chạm timeout 90 giây của bước xem thông tin. Profile thắng được ghi nhớ và dùng lại cho bước tải thật, vì URL media của client cần token sẽ bị 403 nếu thiếu token. Muốn ép một client cụ thể (ví dụ `android_vr`, hiện không cần token nhưng thường chỉ có 360p) thì đặt `CLIPDROP_YOUTUBE_CLIENTS`.
