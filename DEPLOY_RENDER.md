...
- Với link YouTube ở chế độ public, worker thử lần lượt client `android_vr` → `mweb` → `tv` kèm PO token rồi mới quay về client mặc định của yt-dlp. `android_vr` không cần token nên trả lời trong vài giây, trong khi việc tạo token chậm hơn nhiều: đo trên container giới hạn `--cpus=0.1`, `android_vr` xong sau 4 giây còn `mweb` cần khoảng 35 giây, và bản cũ trên Render tiêu hết ngân sách mà không client nào trả lời — vì vậy client không cần token đứng đầu chuỗi. Mỗi profile có trần riêng 20 giây, profile bị bỏ dở không chặn profile sau, và cả chuỗi có ngân sách 45 giây: hết ngân sách thì các client còn lại bị bỏ qua, log ghi `skipped` thay vì để worker chạm timeout 90 giây. Khi mọi profile đều thất bại, người dùng nhận thông báo riêng cho trường hợp máy chủ phản hồi quá chậm. Profile thắng được ghi nhớ và dùng lại cho bước tải thật, vì URL media của client cần token sẽ bị 403 nếu thiếu token. Muốn ép một client cụ thể (ví dụ `mweb,tv` để ưu tiên chất lượng cao) thì đặt `CLIPDROP_YOUTUBE_CLIENTS`.
...
2. Mở **Logs** và tìm dòng `worker raw error:`. Từ bản mới, mỗi lần thử có nhãn client, ví dụ `attempt 1: [android_vr] ...`, `attempt 2: [mweb] ...`, `attempt 3: [tv] ...` — đọc được chính xác client nào bị từ chối và vì sao.
3. `pot_alive: true` nhưng mọi client đều bị từ chối là chặn theo IP: thử lại sau vài giờ, hoặc tạo dịch vụ ở region khác (Render không đổi region tại chỗ, sẽ có URL mới). Bản local ở nhà dùng IP dân dụng nên vẫn là đường ổn định nhất.
4. Nếu log cho thấy `không phản hồi sau 20s` hoặc `skipped: hết ngân sách thời gian thử client` thì lỗi nằm ở CPU chứ không phải YouTube: 0.1 CPU không kịp tạo PO token. Đặt `CLIPDROP_YOUTUBE_CLIENTS=android_vr` để chỉ thử client không cần token, hoặc tắt provider bằng `CLIPDROP_POT_URL=0`.
