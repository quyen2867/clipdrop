...
- Với link YouTube ở chế độ public, worker thử lần lượt client `visionos` → `mweb` → `tv` kèm PO token rồi mới quay về client mặc định của yt-dlp. `visionos` không cần token, trả lời trong 1-2 giây và cung cấp đầy đủ chất lượng (lên đến 4K) mà không bị lỗi HTTP 403 khi tải luồng. Trong khi đó, việc tạo token cho `mweb`/`tv` trên CPU 0.1 mất nhiều thời gian hơn (khoảng 35 giây). Mỗi profile có trần riêng 20 giây và cả chuỗi có ngân sách 45 giây. Profile thắng được ghi nhớ và dùng lại cho bước tải thật. Muốn ép một client cụ thể thì đặt `CLIPDROP_YOUTUBE_CLIENTS` (ví dụ `CLIPDROP_YOUTUBE_CLIENTS=visionos`).
- Hỗ trợ Cookie YouTube qua biến môi trường `CLIPDROP_COOKIES`: Nếu IP datacenter của Render bị YouTube yêu cầu đăng nhập/bot-check ("Sign in to confirm you're not a bot"), xuất file `cookies.txt` từ trình duyệt của bạn (dùng extension như "Get cookies.txt locally") và dán toàn bộ nội dung (hoặc chuỗi base64) vào biến môi trường `CLIPDROP_COOKIES` trên Render Dashboard. yt-dlp sẽ tự động dùng cookie để tải mà không bị chặn.
- Hỗ trợ Proxy qua biến môi trường `CLIPDROP_PROXY`: Nếu IP Render bị chặn triệt để, có thể đặt proxy (ví dụ `http://user:pass@proxy-host:port`) vào biến `CLIPDROP_PROXY` trên Render Dashboard. Worker sẽ tự động kết nối qua proxy này.

Cách đọc kết quả khi YouTube vẫn lỗi, theo thứ tự:

1. Gọi `/api/health`. `pot_alive: false` hoặc `oom_kills > 0` nghĩa là provider bị thiếu RAM (Free chỉ 512 MB). Provider đã được giới hạn heap Node (`--max-old-space-size=128`) để hạn chế OOM.
2. Mở **Logs** và tìm dòng `worker raw error:` để đọc lỗi chi tiết của yt-dlp (ví dụ client nào bị từ chối).
3. Nếu IP datacenter bị chặn bot hoàn toàn: Thêm biến môi trường `CLIPDROP_COOKIES` trên Render Dashboard như hướng dẫn ở trên, hoặc cấu hình `CLIPDROP_PROXY`.
4. Đặt `CLIPDROP_YOUTUBE_CLIENTS=visionos` trong Environment Variables của Render để ưu tiên hoặc ép dùng client không cần token.
